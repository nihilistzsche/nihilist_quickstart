-- luacheck: globals NQS_Init script defines game settings
local function quickstart(e)
	...
	local furnacetype
	if minimachines and settings.global["uqs-mini-furnaces"].value then
		furnacetype = "mini-furnace-1"
	elseif electricfurnaces and settings.global["uqs-electric-furnaces"].value then
	...
	local function getstuff()
		-- Inserting inserters
		if inserternumber and inserternumber > 0 then
			inventory(insertertype, inserternumber)
		end

		-- Inserting belts
		if beltnumber and beltnumber > 0 then
			inventory(belttype, beltnumber)
		end

		-- Inserting undergrounds
		if undergroundnumber and undergroundnumber > 0 then
			inventory(undergroundtype, undergroundnumber)
		end

		-- Inserting splitters
		if splitternumber and splitternumber > 0 then
			inventory(splittertype, splitternumber)
		end

		-- Inserting loaders
		if loadernumber and loadernumber > 0 then
			inventory(loadertype, loadernumber)
		end

		-- Inserting beltboxes
		if beltboxnumber and beltboxnumber > 0 then
			inventory(beltboxtype, beltboxnumber)
		end

		-- Inserting pipes
		if pipenumber and pipenumber > 0 then
			inventory(pipetype, pipenumber)
		end

		-- Inserting pipe-to-grounds
		if pipeundergroundnumber and pipeundergroundnumber > 0 then
			inventory(pipeundergroundtype, pipeundergroundnumber)
		end

		-- Inserting mining drills
		if minernumber and minernumber > 0 then
			inventory(minertype, minernumber)
		end

		-- Inserting mining drones
		if minernumber2 and minernumber2 > 0 and miningdrones then
			inventory(minertype2, minernumber2)
		end

		-- Inserting labs
		if labnumber and labnumber > 0 then
			inventory(labtype, labnumber)
		end

		-- Inserting assembling machines
		if assemblernumber and assemblernumber > 0 then
			inventory(assemblertype, assemblernumber)
		end

		-- Inserting power poles
		if polenumber and polenumber > 0 then
			inventory(poletype, polenumber)
		end

		-- Inserting Omnitractors
		if Omnimatter and omnitractornumber and omnitractornumber > 0 then
			inventory(omnitractortype, omnitractornumber)
		end

		-- Inserting furnaces
		if furnacenumber and furnacenumber > 0 then
			inventory(furnacetype, furnacenumber)
		end

		-- Inserting Market stuff
		if marketmod then
			if marketchestsnumber and marketchestsnumber > 0 then
				inventory(marketcheststype, marketchestsnumber)
			end
			if markettanksnumber and markettanksnumber > 0 then
				inventory(markettankstype, markettanksnumber)
			end
			if marketaccumulatorsnumber and marketaccumulatorsnumber > 0 then
		...
		if oxygenmod then
			if oxygenbottlesnumber and oxygenbottlesnumber > 0 then
		...
		if Angelssmelting then
			local ifurnacenumber = settings.global["uqs-number-of-induction-furnaces"].value
			if ifurnacenumber > 0 then
				inventory("induction-furnace", ifurnacenumber)
			end
			local bfurnacenumber = settings.global["uqs-number-of-blast-furnaces"].value
			if bfurnacenumber > 0 then
				inventory("blast-furnace", bfurnacenumber)
			end
			local cmachinenumber = settings.global["uqs-number-of-casting-machines"].value
			if cmachinenumber > 0 then
				inventory("casting-machine", cmachinenumber)
			end
		end
		...
		if settings.global["uqs-number-of-fuel"].value > 0 then
			...
			elseif settings.global["uqs-fuel-type"].value == "Purified Omnite" then
				inventory("purified-omnite", settings.global["uqs-number-of-fuel"].value)
			end
		end
