...
local function quickstart_bne(e)
    if NQS_BNE_Init and NQS_BNE_Init[e.player_index] then return end
    ...
    if player.controller_type == defines.controllers.character then
        ...
        --	if not settings.global["nqs-custom-entities"].value == "" then
        ...
        local function getarmor()
            ...
            if settings.global["nqs-equipment-in-backpack"].value then inbackpack = true end
            ...
            elseif settings.global["nqs-equipment-power"].value == "Solar panels MK4" then
                ...
                powersourcetype = 1
            elseif settings.global["nqs-equipment-power"].value == "Fusion reactors" then
            ...
            if not noarmor then
                if inbackpack then
                    ...
                    if powersources > 0 then inventory(powertype, powersources) end

                    -- Batteries in backpack
                    if batteries > 0 then inventory(batterytype, batteries) end

                    -- Oxygen gas mask in backpack
                    if oxygenmod and settings.global["nqs-oxygen-gas-mask"].value then inventory("gas-mask", 1) end

                    -- Night vision in backpack
                    if settings.global["nqs-equipment-night-vision"].value then
                    ...
                    if exoskeletons > 0 then inventory(exoskeletontype, exoskeletons) end

                    -- Energy shields in backpack
                    if energyshields > 0 then inventory(energyshieldtype, energyshields) end
                    ...
                    if settings.global["nqs-equipment-belt-immunity"].value then
                    ...
                    for j = 0, (gridwidth - 1) do
                        for i = 0, (gridheight - 1) do
                            ...
                            if powersources > 0 and gridmt[j][i] == 0 then
                                ...
                                elseif powersourcetype == 4 then
                                    ...
                                    if gridwidth > w and gridheight > h then
                                        gridput(powertype, j, i)
                                        powersources = powersources - 1
                                        if fuelcells > 0 then inventory("uranium-fuel-cell", fuelcells) end
                                        for k = j, w do
                                            for l = i, h do
                                                gridmt[k][l] = 1
                                            end
                            ...
                            if
                                oxygenmod
                                and settings.global["nqs-oxygen-gas-mask"].value
                                and not oxygengasmaskplaced
                                and gridmt[j][i] == 0
                            then
                                w = j + 1
                                h = i + 1
                            ...
                            -- Night Vision
                            if
                                settings.global["nqs-equipment-night-vision"].value
                                and not nightvisionplaced
                                and gridmt[j][i] == 0
                            then
                                w = j + 1
                                h = i + 1
                            ...
                            if
                                settings.global["nqs-equipment-belt-immunity"].value
                                and not beltimmunityplaced
                                and gridmt[j][i] == 0
                            then
        ...
        local function getstuff()
            ...
            local roboporttype, fivedimrank
            ...
            log(roboporttype)
            log(roboportsnumber)
            log(settings.global["nqs-number-of-roboports"].value)
            log(settings.global["nqs-type-of-roboports"].value)
            ...
            if giveroboports then
                if roboporttype == 1 then inventory("roboport", roboportsnumber) end
                if roboporttype == 2 then inventory("bob-roboport-2", roboportsnumber) end
                if roboporttype == 3 then inventory("bob-roboport-3", roboportsnumber) end
                if roboporttype == 4 then inventory("bob-roboport-4", roboportsnumber) end
                if roboporttype == 5 then inventory("py-roboport-mk01", roboportsnumber) end
                if roboporttype == 6 then inventory("yi_roboport", roboportsnumber) end
                if roboporttype == 7 then inventory("5d-roboport-" .. fivedimrank, roboportsnumber) end
            end
            if giverobots then
                if logbotsnumber > 0 then inventory(logbots, logbotsnumber) end
        ...
        if not noarmor then player.insert({ name = armorname[armortype], count = 1 }) end
