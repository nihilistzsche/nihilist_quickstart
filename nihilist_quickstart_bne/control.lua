...
local function quickstart_bne(e)
	...
	if (player.controller_type == defines.controllers.character) then
		...
		local function getarmor()
			...
			if settings.global["nqs-equipment-in-backpack"].value then
				inbackpack = true
			end
			...
			if not noarmor then
				if inbackpack then
					...
					if oxygenmod and settings.global["nqs-oxygen-gas-mask"].value then
					...
					for j = 0, (gridwidth - 1) do
						for i = 0, (gridheight - 1) do
							...
							-- Oxygen Gas Mask
							if oxygenmod and settings.global["nqs-oxygen-gas-mask"].value and not oxygengasmaskplaced and gridmt[j][i] == 0 then
								w = j + 1
								h = i + 1
								if gridwidth > w and gridheight > h then
							...
							-- Night Vision
							if settings.global["nqs-equipment-night-vision"].value and not nightvisionplaced and gridmt[j][i] == 0 then
								w = j + 1
								h = i + 1
								if gridwidth > w and gridheight > h then
							...
							if settings.global["nqs-equipment-belt-immunity"].value and not beltimmunityplaced and gridmt[j][i] == 0 then
